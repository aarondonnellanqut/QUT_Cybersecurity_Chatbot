...
WIT_AI_KEY = "YOUR_API_KEY_HERE"

bot_answer = pyttsx3.init()
...
model.fit(training, output, n_epoch=25, batch_size=8,
...
def chat():
    ...
    while True:
        ...
        with sr.Microphone() as source:
            ...
            try:
                resposne = r.recognize_wit(audio, key=WIT_AI_KEY)
                print(f"You: {resposne}")
                inp = resposne
            except sr.UnknownValueError:
                inp = "Wit.ai could not understand audio"
            except sr.RequestError as e:
                raise TypeError(
                    "Could not request results from Wit.ai service; {0}".format(e))
        if inp.lower() == "quit." or inp.lower() == "quit":
            break
        while numpy.sum(bag_of_words(inp, words)) == 0:
            print("I can't recoginze your words, please retry:")
            bot_answer.say("I can't recoginze your words, please retry:")
            bot_answer.runAndWait()
             # obtain audio from the microphone
            r = sr.Recognizer()
            with sr.Microphone() as source:
                print("Listening... ")
        ...
        if (tag == "threats"):
            ...
            print("Ashwin: " + response)
            bot_answer.say(response)
            bot_answer.runAndWait()
...
def industy():
    ...
    bot_answer.say(
        "To help answer this, I'll need know what industry you are in.")
    bot_answer.runAndWait()
    answered = False
    while (answered != True):
        r = sr.Recognizer()
        with sr.Microphone() as source:
            ...
            try:
                ...
                raise TypeError(
                    "Could not request results from Wit.ai service; {0}".format(e))
        if inp.lower() != "":
            response = random.choice(cyberThreats)
            response2 = random.choice(cyberThreats)
            response3 = random.choice(cyberThreats)
            print(
                f"Ashwin: For the {inp} industry, the threats are a mix of {response}, {response2} and {response3}. However here are some search results that you can look at.")
            bot_answer.say(
                f"For the {inp} industry, the threats are a mix of {response}, {response2} and {response3}. However here are some search results that you can look at.")
            bot_answer.runAndWait()
            print("\n")
            ...
            bot_answer.say(
                "You haven't told me an industry. Please try again.")
            bot_answer.runAndWait()
...
def threats():
    ...
    bot_answer.say("To help answer this, I'll need some additional information from your most recent Threat and Risk Assessment.")
    bot_answer.runAndWait()
    answered = False
    while (answered != True):
        print("Ashwin: Do you have a current TRA? (Yes/No)")
        bot_answer.say("Do you have a current T R A? Yes or No?")
        bot_answer.runAndWait()
        r = sr.Recognizer()
        with sr.Microphone() as source:
            print("Listening... ")
        ...
        bot_answer.say("Was the T R A completed in the last 6 months? Yes or No?")
        bot_answer.runAndWait()
        r = sr.Recognizer()
        with sr.Microphone() as source:
            print("Listening... ")
        ...
        if inp.lower() == "no":
            ...
            bot_answer.say(
                "Since the T R A was performed more then 6 months ago, I would strongly recommend that you undertake another T R A ASAP. However, we can still continue.")
            bot_answer.runAndWait()
        ...
        bot_answer.say(
                "Could you please tell me the TRA details of your core information systems, one threat at a time.")
        bot_answer.runAndWait()
        while (coreInformation != True):
         ...
         with sr.Microphone() as source:
            ...
            if inp.lower() == "no":
                ...
                bot_answer.say("Is there anything else you wanted to add? If so, please continue or just say 'no' if you're ready to move on.")
                bot_answer.runAndWait()
            elif numpy.sum(bag_of_words(inp, words)) != 0:
                traResults.append(inp)
                results = model.predict([bag_of_words(inp, words)])
        ...
        print(
            "Ashwin: Thank you for that. Now please tell me what are the TRA details of your supporting people?")
        bot_answer.say("Thank you for that. Now please tell me what are the T R A details of your supporting people?")
        bot_answer.runAndWait()
        ...
        print(
            "Ashwin: Thank you for that. Now please tell me what are the TRA details of your supporting processes?")
        bot_answer.say("Thank you for that. Now please tell me what are the TRA details of your supporting processes?")
        bot_answer.runAndWait()
    ...
    bot_answer.say("Is Thank you for providing the information. So from what I can see, mitigations that I would recommend for your threats are as follows:")
    bot_answer.runAndWait()
    ...
    while (answered != True):
        ...
        bot_answer.say("Would you like to explore the cost of fixing these threats? Yes or No")
        bot_answer.runAndWait()
        r = sr.Recognizer()
        with sr.Microphone() as source:
            print("Listening... ")
        ...
        if inp.lower() == "no":
            ...
            bot_answer.say("No worries, please contact us here if you change your mind.")
            bot_answer.runAndWait()
            answered = True
        elif inp.lower() == "yes":
